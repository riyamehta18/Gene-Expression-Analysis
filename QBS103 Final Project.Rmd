...
all_graphs <- function(data, genes, gene_exp, metadata) {
  ...
  for (gene in genes) {
    gene_data <- as.numeric(t(gene_exp[gene, ]))
    metadata_gene <- cbind(metadata, gene_data)
    metadata_gene <- metadata_gene[metadata_gene$age != " >89" & metadata_gene$age != " :" & metadata_gene$age != "NA", ]
    metadata_gene$age <- as.numeric(metadata_gene$age)

    scatter_outliers <- ggplot(metadata_gene, aes(x = age, y = gene_data)) + 
      geom_point(color = "#6794a7", fill = "white") + 
      labs(x = "Age" , y = paste("Gene Expression Levels of", gene)) + 
      theme_economist() + 
      scale_fill_economist() + 
...
```{r}
#COMBINED SCATTER AND BOX - WORKS
all_graphs <- function(data, genes, gene_exp, metadata) {
  graphs <- list()
  for (gene in genes) {
    gene_data <- as.numeric(t(gene_exp[gene, ]))
    ...
    scatter_outliers <- ggplot(metadata_gene, aes(x = age, y = gene_data)) + 
      geom_point(color = "#6794a7", fill = "white") + 
      labs(x = "Age" , y = paste("Gene Expression Levels of", gene), title = paste("Age vs. Gene Expression Levels of", gene)) + 
      theme_economist() + 
      scale_fill_economist() + 
