...
```{r}
cor.test(metadata_BPI$age, metadata_BPI$BPI)
...
ggplot(metadata_BPI, aes(x = age, y = BPI)) + 
  geom_point(color = greens[4], fill = "white", alpha = 0.8) + 
  ...
  theme_classic() +
  scale_color_manual(values = c(greens[4])) +
...
ggplot(unknown_removed, aes(x = sex, y = BPI, color = MechanicalVentilation)) + 
  ...
  theme_classic() + 
  scale_color_manual(values = c(greens[4], greens[9])) +
  scale_fill_manual(values = "white") +
...
ten_genes <- cbind(metadata, chooseten) #creates one data frame with all ten gene data
...
ten_genes <- ten_genes %>% filter(!grepl('unknown', sex)) #filtering unknown in sex
