...
all_graphs <- function(data, genes, gene_exp, contcovariate, catcovariate1, catcovariate2, metadata) {
  ...
  for (gene in genes) {
    ...
    scatter <- ggplot(metadata_gene, aes(x = contcovariate, y = gene_data)) + 
      geom_point(color = "#6794a7", fill = "white") + 
      labs(x = paste("", contcovariate) , y = paste("Gene Expression Levels of", gene), title = paste("", contcovariate, "vs. Gene Expression Levels of", gene)) + 
      theme_economist() + 
      scale_fill_economist() + 
      ...
      ggtitle(paste("Gene Expression Levels of", gene, "vs.", contcovariate))
    ...
    box <- ggplot(metadata_gene, aes(x = catcovariate1, y = gene_data, color = catcovariate2)) + 
      ...
      labs(x = paste("", catcovariate1), y = paste("Gene Expression Levels of", gene), color = paste("", catcovariate2)) + 
      theme_economist() + 
      scale_color_manual(values = c('#014d64','#6794a7')) + 
      theme(plot.title = element_text(size = 15, face ="bold", margin = margin(10, 0, 10, 0), hjust = (0.5)), axis.title.x = element_text(vjust=-0.40), axis.title.y = element_text(vjust=2)) +
      ggtitle(paste(gene, "Expression by", catcovariate1, "and", catcovariate2))
...
multiple_graphs <- all_graphs(data = gene_exp, genes = selected_genes, contcovariate = "age", catcovariate1 = "sex", catcovariate2 = "disease_status", gene_exp = gene_exp, metadata = metadata)
